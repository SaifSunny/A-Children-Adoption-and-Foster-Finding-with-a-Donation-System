...
if(isset($_POST['submit'])){
    ...
    $query = "SELECT * FROM adoption_meetings WHERE user_id = '$user_id' AND child_id = '$child_id' AND `date` = '$date' AND `time` = '$time'";
    ...
    if(!$query_run->num_rows > 0){
        ...
        $query_run = mysqli_query($conn, $query);
        if(!$query_run->num_rows > 0){
            $query2 = "INSERT INTO adoption_meetings(`user_id`, orphanage_id, child_id, user_image, orphanage_image, child_image, `user_name`, orphanage_name, child_name, `date`, `time`, `message`, contact, email)
            VALUES ('$user_id', '$orphanage_id', '$child_id',(SELECT `user_img` FROM users WHERE user_id='$user_id'), (SELECT `orphanage_img` FROM orphanage WHERE orphanage_id='$orphanage_id'),
            (SELECT `child_img` FROM for_adoption WHERE child_id='$child_id'), '$username', (SELECT `orphanage_name` FROM orphanage WHERE orphanage_id='$orphanage_id'),(SELECT `child_name` FROM for_adoption WHERE child_id='$child_id'), '$date', '$time','$message','$contact','$email')";
